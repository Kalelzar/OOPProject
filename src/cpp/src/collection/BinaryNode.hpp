...
public:
  ...
  std::unique_ptr<ArrayList<A>> allInRangeT(C const &lower,
                                            C const &upper) const {
    std::unique_ptr<ArrayList<A>> range = std::make_unique<ArrayList<A>>();
    // std::std::cout<<"All in range "<<lower<<" to " <<upper<<std::endl;
    if (tag >= lower && tag <= upper) {
      // std::std::cout<<tag<<" is between "<<lower<<" and "<<upper<<std::endl;
      range->appendAll(value);
      if (right != nullptr)
      ...
      // std::std::cout<<tag<<" is greater than "<<upper<<std::endl;
      if (left != nullptr)
        range->appendAll(*left->allInRangeT<C>(lower, upper));
    } else if (tag < lower) {
      // std::std::cout<<tag<<" is less than "<<lower<<std::endl;
      if (right != nullptr)
        range->appendAll(*right->allInRangeT<C>(lower, upper));
    } else {
