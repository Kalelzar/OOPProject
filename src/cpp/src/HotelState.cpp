...
void Hotel::HotelState::checkin(int roomid, Date start, Date end,
  ...
  std::unique_ptr<Nullable<Room>> room = rl.get(roomid);
  if (room->isEmpty()) {
...
Hotel::HotelState &
Hotel::HotelState::operator=(const Hotel::HotelState &other) {
  if (this != &other) {
    delete[] filepath;
    copy(other);
  }
...
Hotel::HotelState::~HotelState() { free(); }
...
void Hotel::HotelState::load() {
  if (strcmp(filepath, "") == 0) {
    ...
    if (file.is_open()) {
      ...
      std::shared_ptr<ArrayList<Token>> fileTokens = fileScanner.scan();
      if (fileScanner.error) {
        std::cerr << "File '" << filepath << "' contains errors: " << std::endl;
        fileTokens
            ->filter(
                [](Token const &token) { return token.t == TokenType::ERROR; })
            ->foreach ([](Token const &token) {
              std::cerr << "Error(" << token.line << ") : " << token.lexeme
                        << std::endl;
            });
        setFile("");
        return;
      ...
      if (interpreter.errorflag) {
        std::cerr << "File could not be opened because it contained errors. "
                  << std::endl;
        clear();
        setFile("");
        open = false;
...
void Hotel::HotelState::unavailable(int roomid, Hotel::Date from,
                                    Hotel::Date to, const char *note) {
  if (!checkOpen())
    return;
  modified = true;
...
void Hotel::HotelState::findForce(int beds, Hotel::Date from, Hotel::Date to) {
  ...
  if (range) {
    ...
    auto notFull = range
                       ->filter([](std::shared_ptr<RoomStateEvent> const &rse) {
                         return rse->state == RoomState::TAKEN;
                       })
                       ->filter([](std::shared_ptr<RoomStateEvent> const &rse) {
                         return rse->extra < rse->room.beds && rse->extra != -1;
                       })
                       ->sort([](std::shared_ptr<RoomStateEvent> const &rse1,
                                 std::shared_ptr<RoomStateEvent> const &rse2) {
                         if (rse1->room.beds == rse2->room.beds)
                           return rse1->extra < rse2->extra;
                         else
                           return rse1->room.beds < rse2->room.beds;
                       });

    // Find all rooms not at full capacity whose occupants
    // Can be moved to a free room that can full accomodate them
    auto directMovable = notFull->filter(
        [this, from, to](std::shared_ptr<RoomStateEvent> const &rse) {
          return findBackend(rse->extra, from, to)->isDefined();
        });

    if (directMovable->length() == 0) {
      std::cerr << "Cannot find a way to fit " << beds << " people in a room. "
                << std::endl;
      return;
    }
    ...
    auto largeEnoughDirectMovable = directMovable->filter(
        [beds](std::shared_ptr<RoomStateEvent> const &rse) {
    ...
    if (largeEnoughDirectMovable->length() > 0) {
      ...
      std::cout << "The occupants of room " << r.id
                << " need to be moved to room " << room.id << "." << std::endl
                << "Then room " << r.id << " with " << r.beds
                << " beds, will be available in the time period from " << from
                << " to " << to << "." << std::endl;
      return;
    }

    // Is there a notFull room that is big enough?
    auto largeEnoughNotFull =
        notFull->filter([beds](std::shared_ptr<RoomStateEvent> const &rse) {
          return rse->room.beds >= beds;
        });
    ...
    for (unsigned i = 0; i < largeEnoughNotFull->length(); i++) {
      ...
      auto directMovableLargeEnoughForNotFull = directMovable->filter(
          [people](std::shared_ptr<RoomStateEvent> const &rse) {
      ...
      if (directMovableLargeEnoughForNotFull->length() > 0) {
        ...
        std::cout << "The occupants of room " << r.id
                  << " need to be moved to room " << room.id << "." << std::endl
                  << "Then the occupants of room " << nfrse.room.id
                  << " need to be moved to room " << r.id << "." << std::endl
                  << "Then room " << nfrse.room.id << " with "
                  << nfrse.room.beds
                  << " beds, will be available in the time period from " << from
                  << " to " << to << "." << std::endl;
        return;
...
std::unique_ptr<ArrayList<Hotel::Room>>
Hotel::HotelState::roomsAvailableFrom(Date from, Date to) const {
  if (from > to) {
    Date t = to;
    to = from;
    from = t;
  }
  std::unique_ptr<ArrayList<std::shared_ptr<RoomStateEvent>>> range =
      tree.inRangeT<Hotel::Date>(from, to);

  if (range) {

    std::unique_ptr<ArrayList<std::shared_ptr<RoomStateEvent>>> filtered =
        range->filter([](std::shared_ptr<RoomStateEvent> const &rse) {
          return rse->state != RoomState::FREE;
        });

    std::unique_ptr<ArrayList<Room>> rse = filtered->template map<Room>(
        [](std::shared_ptr<RoomStateEvent> const &rse) { return rse->room; });

    std::unique_ptr<ArrayList<Room>> freeRooms = rse->difference(*rl.values());
    return freeRooms;
...
std::unique_ptr<Nullable<Hotel::Room>>
Hotel::HotelState::findBackend(int beds, Hotel::Date from,
                               Hotel::Date to) const {

  std::unique_ptr<ArrayList<Room>> list = roomsAvailableFrom(from, to)->filter(
      [beds](Room const &r) { return r.beds >= beds; });

  if (list->length() == 0) {
    return std::make_unique<Null<Room>>();
  } else {
    std::unique_ptr<ArrayList<Room>> slist =
        list->sort([](Room r1, Room r2) { return r1.beds < r2.beds; });

    Room room = slist->get(0);
    return std::make_unique<NotNull<Room>>(room);
  }
}
...
bool Hotel::HotelState::find(int beds, Hotel::Date from, Hotel::Date to) const {
  ...
  if (res->isEmpty()) {
    std::cout << "No suitable rooms found in the given time period."
              << std::endl;
    return false;
  } else {
    Room room = res->get();
...
void reportPrint(std::shared_ptr<Hotel::RoomStateEvent> const &_rse) {
  auto rse = *_rse;
  ...
  if (delta == 1) {
    ...
    std::cout << " from " << rse.from << " to " << rse.to << " for a total of "
              << delta << " days";
    if (rse.extra != -1) {
      std::cout << " by " << rse.extra << " people out of " << rse.room.beds
                << " beds";
    }
  }
...
void Hotel::HotelState::report(Hotel::Date from, Hotel::Date to) {
  ...
  if (range) {
    ...
    std::unique_ptr<ArrayList<std::shared_ptr<RoomStateEvent>>> filtered =
        range
            ->filter([this](std::shared_ptr<RoomStateEvent> const &rse) {
              return rse->state != RoomState::FREE && rl.contains(rse->room.id);
            })
            ->sort([](std::shared_ptr<RoomStateEvent> const &rse1,
                      std::shared_ptr<RoomStateEvent> const &rse2) {
              if (rse1->from == rse2->from)
                return rse1->room.id < rse2->room.id;
              else
                return rse1->from < rse2->from;
            });

    if (filtered->length() == 0) {
      std::cout << "No rooms are in use for the time period from " << from
                << " to " << to << "." << std::endl;
      return;
    }

    std::cout << "The following room"
              << (filtered->length() == 1 ? " is" : "s are") << " in use from "
              << from << " to " << to << "." << std::endl;
...
void Hotel::HotelState::available(Hotel::Date date) const {
  if (!checkOpen())
    return;
  std::unique_ptr<ArrayList<Room>> list =
      roomsAvailableFrom(date, date)->sort([](Room const &r1, Room const &r2) {
        return r1.id < r2.id;
      });
  if (list->length() > 0) {
    std::cout << "The following room" << (list->length() == 1 ? " is" : "s are")
              << " available on " << date << "." << std::endl;
    list->foreach ([](Room const &room) {
      std::cout << "  Room " << room.id << std::endl;
    });
  } else {
...
void Hotel::HotelState::saveAs(const char *path) {
  ...
  if (file.is_open()) {
    ...
    std::cout << ".";
    Hotel::HTLPrettyPrinter::printState(
        tree.getHead()
            ->collect()
            ->asList()
            ->map<ArrayList<std::shared_ptr<RoomStateEvent>>>(
                [](BinaryNode<DateRange, std::shared_ptr<RoomStateEvent>>
                       *node) { return node->value; })
            ->flatten<std::shared_ptr<RoomStateEvent>>(),
        &file);
    std::cout << ".";
    file.close();
    std::cout << std::endl;
...
const char *Hotel::HotelState::getFile() const { return filepath; }
...
Hotel::HotelState::setRoomState(Hotel::RoomState state, int roomid,
  ...
  std::unique_ptr<ArrayList<std::shared_ptr<RoomStateEvent>>> events =
      tree.inRangeT<Hotel::Date>(start, end);
  bool cannotCheckIn =
      events && events
                    ->filter([](std::shared_ptr<RoomStateEvent> const &rse) {
                      return rse->state != RoomState::FREE;
                    })
                    ->map<Room>([](std::shared_ptr<RoomStateEvent> const &rse) {
                      return rse->room;
                    })
                    ->contains(room->get());
  ...
  if (cannotCheckIn) {
    throw InvalidArgumentException(
        "Room with the provided number is taken or unavailable.");
  }

  auto rse = std::make_shared<RoomStateEvent>(state, room->get(), start, end,
                                              note, extra);
