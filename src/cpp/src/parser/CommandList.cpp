...
Hotel::CommandList &
Hotel::CommandList::operator=(Hotel::CommandList const &other) {
  if (&other != this) {
    copy(other);
  }
  return *this;
...
void Hotel::CommandList::printCommandsWithDescription() {
  std::cout << "Simple Command-line interface for Hotel Management"
            << std::endl;
...
Hotel::CommandList::contextFor(TokenType tokenType) {
  return commandMap.get(tokenType);
}

std::unique_ptr<Nullable<TokenType>>
Hotel::CommandList::tokenFor(SimpleString name) {
