...
private:
  ...
  void add(HotelState &state) {
    if (matches(TokenType::NUMBER)) {
      ...
      error(peek().line,
            "add <room> <beds> : <room> should be a number or a numeric range");
    }
  }
  ...
  void remove(HotelState &state) {
    if (matches(TokenType::NUMBER)) {
      ...
      error(peek().line,
            "remove <room> : <room> should be a number or a numeric range");
    }
  }

  void checkout(HotelState &state) {
    Token roomid = consume(
        TokenType::NUMBER,
  ...
  void checkin(HotelState &state) {
    if (matches(TokenType::NUMBER)) {
      ...
      Token from =
          consume(TokenType::DATE, "checkin <room> <from> <to> <note> [people] "
                                   ": <from> should be date.");
      ...
      Token note =
          consume(TokenType::STRING, "checkin <room> <from> <to> <note> "
                                     "[people]: <note> should be string.");
      ...
      error(peek().line, "checkin <room> <from> <to> <note> [people] : <room> "
                         "should be a number or a numeric range");
    }
  }
  ...
  void unavailable(HotelState &state) {
    if (matches(TokenType::NUMBER)) {
      ...
      error(peek().line, "unavailable <room> <from> <to> <note> : <room> "
                         "should be a number or a numeric range");
    }
  }
  ...
  void report(HotelState &state) {
    ...
    Token to =
        consume(TokenType::DATE, "report <from> <to> : <to> should be a date");
  ...
  void save(HotelState &state) { state.save(); }

  void saveAs(HotelState &state) {
    Token path =
        consume(TokenType::STRING, "saveas <path> : <path> should be string");
    state.saveAs(path.lexeme);
  }

  void open(HotelState &state) {
    Token path =
        consume(TokenType::STRING, "open <path> : <path> should be string");
    state.setFile(path.lexeme);
    state.load();
  }

  void close(HotelState &state) { state.close(); }
  ...
  void list() {
    ...
    for (const auto &entry : std::filesystem::directory_iterator(path)) {
      ...
      if (epath.extension().string().compare(".htl") != 0 &&
          epath.extension().string().compare(".htl~") != 0)
        continue;
    ...
    while (!isAtEnd()) {
      try {
        ...
        case TokenType::NUMERIC_RANGE:
          errorflag = true;
          std::cerr << "(Line " << prev().line << ") "
                    << "Unexpected Numeric Range -> " << prev().lexeme
                    << std::endl;
          break;
