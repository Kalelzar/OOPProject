...
public:
   ...
   */
  static CommandList getCommandList() { return globalCommandList; }

  /**
   * Sets the global command list.
   * @param cl the new command list
   */
  static void setCommandList(CommandList const &cl) { globalCommandList = cl; }
  ...
  void registerCommand(SimpleString name, TokenType tokenType,
                       ScannerContext ctx, SimpleString descr);
