 ...
 *
 * You can provide a list of reserved keywords with {@link Hotel::CommandList
 * CommandList}. Some keywords may only be provided in a certain context (File,
 * Command-line or all) as such the scanner needs to know it's own context which
 * is provided at instantiation through {@link ScannerContext}
 *
 */
class Scanner {
private:
...
public:
   ...
   * TokenType::EOF_T EOF}
   *
   * @return a std::shared pointer to a list containing the resulting tokens
   */
