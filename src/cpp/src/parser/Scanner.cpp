...
Hotel::Scanner::Scanner(ScannerContext sc, std::istream *input,
                        CommandList const &cl) {
  this->sc = sc;
  this->input = input;
  this->cl = cl;
}

std::shared_ptr<ArrayList<Token>> Hotel::Scanner::scan() {
  error = false;
...
std::shared_ptr<ArrayList<Token>> Hotel::Scanner::scanNext() {
  error = false;
  std::shared_ptr<ArrayList<Token>> list = std::make_shared<ArrayList<Token>>();
  char sline[513];
  input->getline(sline, 512);
...
std::shared_ptr<ArrayList<Token>> Hotel::Scanner::scanLine(const char *line) {
  std::shared_ptr<ArrayList<Token>> list = std::make_shared<ArrayList<Token>>();
