...
Hotel::Scanner::Scanner(ScannerContext sc, std::istream *input, CommandList const &cl)
{
    this->sc = sc;
    this->input = input;
    this->cl = cl;
...
shared_ptr<ArrayList<Token>> Hotel::Scanner::scan(){
    error = false;
...
shared_ptr<ArrayList<Token>> Hotel::Scanner::scanNext(){
    error = false;
    shared_ptr<ArrayList<Token>> list = make_shared<ArrayList<Token>>();
    char sline[513];
    input->getline(sline, 512);
...
shared_ptr<ArrayList<Token>> Hotel::Scanner::scanLine(const char *line){
    shared_ptr<ArrayList<Token>> list = make_shared<ArrayList<Token>>();
