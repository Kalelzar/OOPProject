...
int main() {
    ...
    const char* addHelp=" <room> <beds> : Add a new room to the database. (File only)";
    ...
    const char* checkinHelp=" <room> <from> <to> <note> : Marks <room> as taken"
      "for the period <from> <to>. Also attaches a <note> to the check-in event.";
    const char* checkoutHelp=" <room> [date]: If the room is marked taken for a period that includes [date] (defaults to today)\n"
        " then mark the room as free from <date> until the end of the period.";
    const char* unavailableHelp=" <room> <from> <to> <note> : Marks <room> as unavailable for the period <from> <to>. Also attaches a <note> to the unavailable event.";
    const char* reportHelp=" <from> <to> : Display when rooms are taken or unavailable for the period <from> <to> (Console only)";
    const char* findHelp=" <beds> <from> <to> : Find a room with at least <beds> beds that is free <from> <to> (Console only)";
    const char* findFHelp=" <beds> <from> <to> : Find a room with at least <beds> beds that can be free <from> <to> if the occupants of at most two rooms are moved (Console only)";
    const char* availabilityHelp=" [date] : show all rooms that are free on [date] (defaults to today) (Console only)";
    const char* openHelp=" <filepath> : Open file at <filepath> (Console only)";
    const char* closeHelp=" : Close currently opened file. Does not save the file. (Console only)";
    ...
    cl.registerCommand("remove", TokenType::REMOVE, ScannerContext::FILE, removeHelp);
    ...
    cl.registerCommand("availability", TokenType::AVAILABILITY, ScannerContext::CONSOLE, availabilityHelp);

    cl.registerCommand("open", TokenType::OPEN, ScannerContext::CONSOLE, openHelp);
    cl.registerCommand("close", TokenType::CLOSE, ScannerContext::CONSOLE, closeHelp);
    cl.registerCommand("help", TokenType::HELP, ScannerContext::CONSOLE, helpHelp);
    cl.registerCommand("exit", TokenType::EXIT, ScannerContext::CONSOLE, exitHelp);
    cl.registerCommand("save", TokenType::SAVE, ScannerContext::CONSOLE, saveHelp);
    cl.registerCommand("saveas", TokenType::SAVE_AS, ScannerContext::CONSOLE, saveasHelp);

    // Mark the command list as global so it isn't necessary to lug it around everywere
    ...
    while(run){
        if(strlen(state.getFile()) != 0){
            std::cout<<"["<<state.getFile()<<"]"; // Print out the name of the opened file if any
        }
        std::cout<<"$ ";
